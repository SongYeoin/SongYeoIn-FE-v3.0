...
//
//  return (
//    <aside className="w-56 bg-[#ffffff] flex flex-col gap-4 p-6 fixed top-16 left-3 bottom-3 rounded-[10px]">
//      <nav className="flex flex-col gap-4 fs-2">
//        {menuItems.map((item, index) => (
//          <div
//            key={index}
//            className={`gap-3 p-3 rounded-[8px] transition-all duration-200
//              ${location.pathname === item.link
//                ? 'bg-gray-100'  // 그림자 대신 회색 배경으로 변경
//                : 'hover:bg-gray-100'}`}  // hover 스타일도 동일하게 변경
//          >
//            <a
//              href={item.link}
//              className={`flex items-center justify-start gap-5 ml-4
//                ${location.pathname === item.link
//                  ? 'text-black opacity-100'  // 활성 메뉴일 때 검정색
...
const AdminSidebar = () => {
  ...
  return (
    <aside className="w-56 bg-[#D4E2D4] flex flex-col gap-4 p-6 fixed top-[74px] left-3 bottom-3 rounded-[10px]">
      <nav className="flex flex-col gap-4 fs-2">
        {menuItems.map((item, index) => (
          <div
            key={index}
